...
import { findTripGoToCalendarKeyboard, blockedFindTripCitiesKeyboard } from '../../keyboards/keyboards';
import {
    sendMessage,
    sendLocation,
    getCityDetails,
...
export const sendBlockedFindTripCityMessage = (bot, msg) => {
    const { chat: { id } } = msg;
    sendMessage(bot, id, getLocalizedMessage(keysActions.FIND_TRIP_GO_TO_CALENDAR_BLOCKED_MESSAGES_KEY, msg));
};

const getCitiesButton = ({ data, place_id, nextCityIndex, allCitiesSize, eventObject }) => {
    const inline_keyboard = nextCityIndex ?
        [
            [{
                text: getLocalizedMessage(keysActions.CHOOSE_FIND_TRIP_CITY_MESSAGES_KEY, eventObject),
                callback_data: createCityAction(getLocalizedMessage(keysActions.CHOOSE_FIND_TRIP_CITY_MESSAGES_KEY, eventObject), place_id),
            }],
            [{
                text: `${getLocalizedMessage(keysActions.SHOW_NEXT_CITY_KEY, eventObject)} (${allCitiesSize})`,
                callback_data: createNextCityAction(getLocalizedMessage(keysActions.CHOOSE_FIND_TRIP_CITY_MESSAGES_KEY, eventObject), data.text, nextCityIndex),
            }]
        ] : [
            [{
                text: getLocalizedMessage(keysActions.CHOOSE_FIND_TRIP_CITY_MESSAGES_KEY, eventObject),
...
export const startTripCitiesSearching = async (bot, msg) => {
    const { chat: { id } } = msg;
    await sendMessage(bot, id, getLocalizedMessage(keysActions.CITIES_INITIAL_HELP_MESSAGES_KEY, msg), { parse_mode: 'HTML', ...blockedFindTripCitiesKeyboard(msg) });
...
export const addCityToFindTripCities = async (bot, query) => {
    ...
    if (isMoreThenMAxCitiesNumber) {
        return sendMessage(bot, id, getLocalizedMessage(keysActions.FIND_TRIP_MAX_CITIES_CONT_MESSAGES_KEY, query));
    }

    if (isAlreadyAdded) {
        return sendMessage(bot, id, getLocalizedMessage(keysActions.FIND_CITY_ALREADY_EXISTS_ERROR_MESSAGES_KEY, query));
    }

    if (canBeTheFinalCity) {
        await toggleIsFindTripCitiesCreating(id, false);
        sendMessage(bot, id, getLocalizedMessage(keysActions.FIND_TRIP_CITIES_ADD_NEW_HELP_MESSAGES_KEY, query), findTripGoToCalendarKeyboard(query));
        await addCityToFindTripInDB(id, newCity);
    } else {
        sendMessage(bot, id, getLocalizedMessage(keysActions.FIND_TRIP_CITIES_ADD_NEW_HELP_MESSAGES_KEY, query));
        await addCityToFindTripInDB(id, newCity);
    }
};
...
export const handlefindTripsShowCities = async ({ bot, data, customCityIndex, eventObject }) =>  {
    ...
    if (isCityFounded) {
        ...
        await sendMessage(bot, data.id, getLocalizedMessage(keysActions.FIND_TRIPS_NOT_FOUND_CITY_MESSAGES_KEY, eventObject));
    }
};

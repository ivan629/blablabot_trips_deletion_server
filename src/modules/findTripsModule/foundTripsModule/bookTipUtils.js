...
export const bookTripInDb = async (query, trip) => {
    ...
    let {
        book: { booked_seats_count, available_seats_count },
        trip_id,
    } = trip;

    const newBookedSeatsCount =
        booked_seats_count === available_seats_count
            ? available_seats_count
            : booked_seats_count + 1;
    await updateFieldInUserDoc(chat_id, `booked_trips_ids.${trip_id}`, trip_id);
    await updateAvailableSeatsInTip(
        trip_id,
        available_seats_count,
        newBookedSeatsCount,
        chat_id
...
export const unBookTripInDb = async (query, trip) => {
    ...
    const newBookedSeatsCount =
        booked_seats_count === 0 ? 0 : booked_seats_count - 1;
    await removeFieldInCollection(
        chat_id,
        `booked_trips_ids.${trip_id}`,
...
export const editTripMessage = async (bot, query, isAfterBooked) => {
    ...
    const tripHtml = await getTripHtmlSummary({
        trip,
    ...
    const keyBoard = myTripsTripActionKeyboard({
        trip_id,
...
const sendNotificationToUser = async (bot, trip, query, isBooked) => {
    ...
    const phoneNumbers = await getFieldFromDoc(
        chat.id,
        'carrier.phone_numbers'
    );
    const formattedPhoneNumbers = Object.values(
        phoneNumbers
    ).map((phoneNumber) => getFormattedPhoneNumber(phoneNumber));
    const message = isBooked
        ? `👤 <b>${chat.first_name} ${chat.last_name}</b> ${getLocalizedMessage(
              keysActions.BOOK_TRIP_USER_NOTIFICATION_MESSAGES_KEY,
              query
          )} <b>${getFormattedCities(trip, query)}</b>`
        : `❌ <b>${chat.first_name} ${chat.last_name}</b> ${getLocalizedMessage(
              keysActions.CANCEL_TRIP_BOOKING_USER_NOTIFICATION_MESSAGES_KEY,
              query
          )} <b>${getFormattedCities(trip, query)}</b>`;
    const phones = `☎️ <b>${getLocalizedMessage(
        keysActions.CONTACT_NUMBER_MESSAGES_KEY,
        query
    )}</b> ${formattedPhoneNumbers}`;

...
import { TIME_CHOOSING_MESSAGE, CONFIRM_TRIP_DATE} from '../../common/constants/commonСonstants';
import { getNotCompletedTrip, updateFieldDb, toggleIsTripCitiesCreating } from '../../services/helpers';
...
export const userChangedDate = async (query, bot) => {
    ...
    if (isNil(notCompletedTrip)) {
        ...
        await updateFieldDb(chat_id, `trips.${trip_id}.start_date.start_date_day`, start_date_day);
        await updateFieldDb(chat_id, `trips.${trip_id}.start_date.start_date_month`, start_date_month);
        await updateFieldDb(chat_id, `trips.${trip_id}.start_date.start_date_year`, start_date_year);
    }

    await sendCurrentDateHtml(chat_id, bot, calendarKeyboard);
};
...
export const setTripHour = async (query, bot) => {
    ...
    await updateFieldDb(id, `trips.${trip_id}.start_date.start_date_hour`, parseData(data).payload);
    await sendCurrentDateHtml(id, bot);
};
...
export const setTripMinutes = async (query, bot) => {
    ...
    const { trip_id } = await getNotCompletedTrip(id);
    await updateFieldDb(id, `trips.${trip_id}.start_date.start_date_minutes`, parseInt(parseData(data).payload));
    await sendCurrentDateHtml(id, bot);
};

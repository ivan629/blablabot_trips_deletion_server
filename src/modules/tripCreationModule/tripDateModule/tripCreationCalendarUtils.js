...
import {
    ...
    getDateMilliseconds,
} from '../../../common/components/calendarComponent/calendarComponentUtils';
import {
...
export const sendCurrentDateHtml = async (
    id,
    bot,
    calendarKeyboard,
    isOnlyDate
...
export const getTripCreationMinCalendarDateThreshold = async (chat_id) => {
    ...
    if (isStartDateCreatingCompleted) {
        ...
        const minDay =
            start_date_hour < 24 ? start_date_day - 1 : start_date_day;

        // TODO: we add + one to fix a bug with isDayValid func
        const minMonth = start_date_month + 1;
        minDateMillisecondsThreshold = getDateMilliseconds(
            minDay,
            minMonth,
            +start_date_year
        );
...
export const showTimeComponent = async (bot, msg) => {
    ...
    sendMessage(
        bot,
        id,
        getLocalizedMessage(TIME_CHOOSING_MESSAGE_KEY, msg),
        timePicker
    );
...
const setDatePickerDataToDb = async (chat_id, field, data) => {
    ...
    const tripDateType = isStartDateCreatingCompleted
        ? 'stop_date'
        : 'start_date';
    const readyField = isStartDateCreatingCompleted
        ? `stop_date_${field}`
        : `start_date_${field}`;
    await updateFieldInUserDoc(
        chat_id,
        `create_trip.${tripDateType}.${readyField}`,
        data
    );
};
...
export const setTripHour = async (query, bot) => {
    ...
    await sendCurrentDateHtml(
        chat_id,
        bot,
        calendarKeyboard(keyboardType, query)
    );
};
...
export const showBlockedGoToTimePickerMessage = (bot, msg) => {
    ...
    sendMessage(
        bot,
        id,
        getLocalizedMessage(BLOCKED_GO_TO_TIME_PICKER_MESSAGE_KEY, msg)
    );
};

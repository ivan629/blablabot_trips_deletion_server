...
export const timeComponent = async (chat_id) => {
    ...
    const hours = new Array(24)
        ...
        .reduce((result, item, calendarHour) => {
            ...
            const text =
                formattedCalendarHour > 12
                    ? `${formattedCalendarHour} pm`
                    : `${formattedCalendarHour} am`;

            result.push({
                text,
                callback_data: createAction(
                    SET_TRIP_HOUR,
                    formattedCalendarHour
                ),
            });
    ...
    return {
        reply_markup: {
            inline_keyboard: [...chunk(hours, 6), minutes],
        },
    };
};

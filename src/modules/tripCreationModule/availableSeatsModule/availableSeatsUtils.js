...
export const setAvailableSeatsData = async (bot, query) => {
    ...
    const config = {
        parse_mode: 'HTML',
        ...availableSeatsCongratsKeyboard(query),
    };
    await sendMessage(
        bot,
        id,
        getLocalizedMessage(
            keysActions.CHOSEN_AVAILABLE_SEATS_MESSAGES_KEY,
            query
        )(payload),
        config
    );
...
export const sendInitialData = async (bot, msg) => {
    await sendMessage(
        bot,
        msg.chat.id,
        getLocalizedMessage(keysActions.AVAILABLE_SEATS_MESSAGE_KEY, msg),
        availableSeatsKeyboard(msg)
    );
    await sendMessage(
        bot,
        msg.chat.id,
        getLocalizedMessage(
            keysActions.CHOOSE_AVAILABLE_SEATS_MESSAGES_KEY,
            msg
        ),
        availableSeatsKeyboardBlocked(msg)
    );
};
...
export const sendAvailableSeatsBlockedMessage = async (bot, msg) => {
    await sendMessage(
        ...
        msg.chat.id,
        getLocalizedMessage(
            keysActions.AVAILABLE_SEATS_BLOCKED_MESSAGE_KEY,
            msg
        )
    );
};

...
function crateFileLogTransportIfEnabled() {
  const logFilePath = getLogFilePath(true /* create if enabled and do not exists yet */);
  if (!logFilePath) {
    return null;
  }
...
const logger = winston.createLogger({
  transports: compact([crateConsoleLogTransportIfEnabled(), crateFileLogTransportIfEnabled()]),
  format: winston.format.combine(

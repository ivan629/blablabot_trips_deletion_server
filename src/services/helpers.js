...
export const removeSessionMessagesIds = async (bot, chat_id) => {
    ...
    const deleteMessagesReqs = Object.values(messagesIds).map((message_id) =>
...
export const getTrip = async (trip_id) =>
    await getDoc(trip_id, API_CONSTANTS.DB_TRIPS_COLLECTION_NAME);
...
export const setNewDocToTripsLinkerCollection = (docName, data) => {
    firestore
        .collection(API_CONSTANTS.BLA_BLA_CAR_LINKER_TRIPS)
        .doc(docName.toString())
        .set(data)
        .then(() => {
            console.count('Document successfully written!');
...
export const setNewDocToTripsCollection = (docName, data) => {
    firestore
        .collection(API_CONSTANTS.DB_TRIPS_COLLECTION_NAME)
        .doc(docName.toString())
        .set(data)
        .then(() => {
            console.count('Document successfully written!');
...
export const updateAvailableSeatsInTip = (
    docName,
    availableSeatsCount,
    bookedSeatsCount,
    book_user_id,
    ...
    firestore
        ...
        .update({
            ...
            [`book.booked_users_ids.${book_user_id}`]: shouldRemoveUserId
...
export const getFieldFromDoc = async (
    docName,
    filedPath,
...
export const getFieldFromTripsDoc = async (
    docName,
    filedPath,
    defaultValue = undefined
) => {
...
export const getFindTripDate = async (chat_id) => {
    return await getFieldFromDoc(chat_id, 'find_trip.date', {
        day: null,
        month: null,
        year: null,
    });
...
export const getAllTrips = async (chat_id) => {
    const alreadyUploadedRestaurantSnapshot = await firestore
        .collection(API_CONSTANTS.DB_USERS_COLLECTION_NAME)
...
export const getCurrentTripCreationDate = async (chat_id) => {
    ...
    const isStartDateCreatingCompleted = await getIsStartDateCreatingCompleted(
        chat_id
    );
...
export const getIsTripCitiesCreating = async (docName) => {
    const carrierData = await firestore
        .collection(API_CONSTANTS.DB_USERS_COLLECTION_NAME)
        .doc(docName.toString())
        .get();
...
export const getCurrentTripDateText = async (docName, isOnlyDate) => {
    const { day, hour, month, minutes } = await getCurrentTripCreationDate(
        docName
    );
    ...
    return `📅 ${formattedDay}/${formattedMonth}   ⏰ ${
        isOnlyDate ? '🤷‍♀️' : `${formattedHour}:${formattedMinutes}`
    }`;
};
...
export const getCreatingTrip = async (docName) =>
    await getFieldFromDoc(docName, 'create_trip');
...
export const saveTripInDb = async (docName) => {
    const { carrier, create_trip } = await getDoc(
        docName,
    ...
    await updateFieldInUserDoc(
        docName,
        `trips.${create_trip.trip_id}`,
        create_trip.trip_id
    );
};

const saveTripToLinkerCollection = async (
    docName,
    { trip_id, cities, start_date }
) => {
    const sortedCities = getSortedCities(Object.values(cities));
    const reqs = sortedCities
        .slice(0, -1)
        .map(async ({ place_id, name, order }, index) => {
            const linkShortId = shortId.generate();
            const data = {
                start_date,
                trip_id,
                cities,
                current_city_order: order,
            };

            const alreadyExists = await getIfExistDoc(
                place_id,
                API_CONSTANTS.BLA_BLA_CAR_LINKER_TRIPS
            );
            if (alreadyExists) {
                await updateFieldInTipsLinkerCollection(
                    place_id,
                    `${trip_id}.${linkShortId}`,
                    data
                );
            } else {
                await setNewDocToTripsLinkerCollection(place_id, {
                    [trip_id]: { [linkShortId]: data },
                });
            }
        });
...
export const getIfExistDoc = async (
    docName,
    collectionName = API_CONSTANTS.DB_USERS_COLLECTION_NAME
) => {
    const alreadyUploadedRestaurantSnapshot = await firestore
...
export const toggleIsTripStartDateCompleted = async (chat_id) => {
    updateFieldInUserDoc(
        chat_id,
        'create_trip.start_date.is_start_date_completed',
        true
    );
};

export const getIsStartDateCreatingCompleted = async (chat_id) => {
    return await getFieldFromDoc(
        chat_id,
        'create_trip.start_date.is_start_date_completed',
        false
    );
...
export const setAvailableSeatsDataInDB = async (chat_id, data) => {
    await updateFieldInUserDoc(
        chat_id,
        'create_trip.book.available_seats_count',
        data
    );
};
...
export const toggleIsFindTripCitiesCreating = async (chat_id, data) => {
    await updateFieldInUserDoc(
        chat_id,
        'bot.is_find_trip_cities_creating',
        data
    );
};
...
export const addSessionMessagesIdsToDb = async (chat_id, message_id) => {
    await updateFieldInUserDoc(
        chat_id,
        `bot.session_messages_ids.${message_id}`,
        message_id
    );
};

export const getSessionMessagesIds = async (chat_id) =>
    await getFieldFromDoc(chat_id, `bot.session_messages_ids`);

export const clearSessionMessagesIdsInDb = async (chat_id) =>
    await updateFieldInUserDoc(chat_id, `bot.session_messages_ids`, {});

export const getCreatingTripCities = async (chat_id) =>
    await getFieldFromDoc(chat_id, 'create_trip.cities', {});

export const saveCarrierPhoneNumberToDb = async (chat_id, phoneNumber) => {
    await updateFieldInUserDoc(
        chat_id,
        `carrier.phone_numbers.${phoneNumber}`,
        phoneNumber
    );
};
...
export const removeTripFromDb = async (chat_id, trip_id_to_remove) => {
    ...
    const newTripsIds = Object.values(userTripsIds).reduce(
        (result, trip_id) => {
...
export const getIsTripCreatingInProgress = async (chat_id) =>
    await getFieldFromDoc(chat_id, 'bot.is_trip_creating_in_progress', false);

export const getIsFindTripCreatingInProgress = async (chat_id) =>
    await getFieldFromDoc(chat_id, 'bot.is_find_trip_in_progress', false);

export const toggleIsTripCreatingInProgress = async (chat_id, value) =>
    await updateFieldInUserDoc(
        chat_id,
        'bot.is_trip_creating_in_progress',
        value
    );

export const toggleIsFindTripProgress = async (chat_id, value) =>
    await updateFieldInUserDoc(chat_id, 'bot.is_find_trip_in_progress', value);

export const setLanguageToDb = async (chat_id, language) => {
    await updateFieldInUserDoc(chat_id, 'bot.language.', language);
};

export const getLanguage = async (chatId) =>
    await getFieldFromDoc(chatId, 'bot.language');
